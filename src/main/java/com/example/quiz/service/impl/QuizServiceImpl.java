...
public class QuizServiceImpl implements QuizService {
	...
	public BasicRes createOrUpdate(CreateOrUpdateReq req) {
		...
		try {
			...
			if (req.getId() > 0) {
				// req.getId() > 0 表示 req 中的 id 有帶值
				// 以下兩種方式擇一
				// 使用方式一，透過 findById，若有資料，就會回傳一整筆的資料(可能資料量會較大)
				// 使用方法二的話，因為是透過 existsById 來判斷資料是否存在，所以回傳的資料永遠都只會是一個 bit(0 或 1)
...
//			quizDao.save(quiz);
	...
	private BasicRes checkParams(CreateOrUpdateReq req) {
		...
		// 前面加個 ! 表示反向的意思，若字串的檢查結果是 false 的話，就會進到 if 的實作區塊
		...
		// req.getStartDate().isAfter(LocalDate.now()): 若 req 中的開始時間比當前時間晚，會得到 ture
		// !req.getStartDate().isAfter(LocalDate.now()): 前面有加 ! ，表示會得到相反的結果，就是開始時間
		// 會等於小於當前時間
	...
	public BasicRes delete(DeleteReq req) {
		...
		if (!CollectionUtils.isEmpty(req.getIdList())) {
			...
			try {
				...
				// 但實際上也沒刪除任何資料，因此就不需要對這個 Exception 做處理
			}
