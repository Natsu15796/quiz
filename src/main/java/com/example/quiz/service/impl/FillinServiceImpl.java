...
public class FillinServiceImpl implements FillinService {
	...
	public BasicRes fillin(FillinReq req) {
		...
		try {
			...
			for (Question item : quList) { // 1(必) 2(必) 3(選)
				...
				for (Fillin fillin : fillinList) { // 1 3
					...
					// 上面的程式碼除了對 question_id 和 answer 檢查，所以其餘的屬性內容可能是不合法的
				...
				// 每跑完一個問題後正常的情況是: 問題是必填，然後又有回答，每跑完一題，qIds 就會包含該必填問題的 id
		...
		responseDao.save(new Response(req.getQuizId(), req.getName(), //
